const style = {
    base: {
        color: '#fff',
        fontWeight: 600,
        fontFamily: 'Quicksand, Open Sans, Segoe UI, sans-serif',
        fontSize: '16px',
        fontSmoothing: 'antialiased',
        
        ':focus': {
            color: '#424770'
        },

        '::placeholder': {
            color: '#9BACC8',
        },

        ':focus::placeholder': {
            color: '#CFD7DF',
        },

    },

    invalid: {
        color: 'FF0000',

        ':focus': {
            color: '#FA755A',
        },

        '::placeholder': {
            color: '#FFCCA5',
        },
    }
};

const fonts = [
    {
        src: 'https://fonts.googleapis.com/css?family=Source+Code+Pro',
    }
]
const formElements = new POS.Fields("798a672c-8719-47bc-b4a0-0f623f4a666f", {
    fonts
})

/*
The placeholders object is just one example of an additional option you can pass when 
instantiating a field.
*/

var result;
document.getElementById('payment-form').addEventListener('submit', async(event) => {
    
    event.preventDefault()
    const additionalData = {
        holder_name: document.getElementById('cardholder-name').value // This field is mandatory
    }
    result = await POS.createToken(cardNumber, {
        additionalData,
        environment: 'test' // Set the PaymentOS environment you're connecting to
    })
    console.log("The response is: " + result)

})

document.getElementById('payment-form').addEventListener('submit', async(event) => {
    event.preventDefault()
    var json = JSON.parse(result)
    for (var key in json){
        console.log("key: " + key + " value: " + json[key])
    }
    result = await createRequest()
    console.log("Payment response: " + result)
})

async function createRequest() {
    var request = new XMLHttpRequest();
    request.open('POST', '/authorizations');
    request.setRequestHeader("Content-Type", "application/json");
    request.setRequestHeader('api-version', '1.3.0');
    request.setRequestHeader('x-payments-os-env', 'test');
    request.setRequestHeader("app-id", "co.Anthara.testbusinessunit");
    request.setRequestHeader("private-key", "60055ba6-92ad-4a5a-8fec-e64e6d2d208c");
    request.setRequestHeader("Access-Control-Allow-Origin", "*");
    request.setRequestHeader("Access-Control-Allow-Credentials", "true");
    request.setRequestHeader("Access-Control-Allow-Methods", "GET,HEAD,OPTIONS,POST,PUT");
    request.setRequestHeader("Access-Control-Allow-Headers", "Content-Type, Access-Control-Allow-Headers, Origin, Accept, X-Requested-With, Content-Type, Access-Control-Request-Method, private-key, idempotency-key, Authorization");
    
    var randomInt = Math.floor(Math.random() * 1000);
    request.setRequestHeader('idempotency-key', 'cust-'+ randomInt + '-payment');
    var body = {
        'payment_method': {
            'token': token,
            'type': 'tokenized'
        }
    };
    
    console.log("Request body1: " + JSON.stringify(body))
    request.send(JSON.stringify(body));
}